getgenv().credits = -- decided to release open-source, just credit me if youre gonna use this anywhere please :) // Bunu Kullanıcaksan Credit Ver Lütfennn :)
[[FE UFO script made by itszw#2241 // script itszw#2241 Tarafından Yapılmıştır
required hat // istenen hat: roblox.com/catalog/8151404994 (requires amazon prime to redeem)
]]


getgenv().SendChatMessages = false -- set to true to say the Ufo messages in chat (kinda spammy) // Ufo Mesajları Atar (spam Gibidir)
getgenv().PlayAudios = false -- set to true if you want to hear the clientsided sounds // Ufo Soundları Oynatır (Beyin Bozar)

--[[ Controls Kontroller:
    T - toggle stand-by mode // Bekleme Modu
    G - gun mode, doesnt really do anything // Silah Modu Client Side
    B - toggle control mode // Uzaktan Kontrol Etme
    UFO controls // Ufo Kontrolleri:
    WASD - move around // WASD - Etrafta Gezinmeni Sağlar
    QE - rotate left/right // Q Ve E - Sağ Sola Donmeni Sağlar
    FC - go up/down // F Ve C Yukarı Aşşağı Yapar
]]--
...
if not getgenv().credits or getgenv().credits ~= creditsstring then
    game.Players.LocalPlayer:Kick("Dont remove credits Creditleri Silemezsin <itszw#2241")
    wait(.5)
    while true do end
end
...
if PlayAudios then
    ...
    s.SoundId = "http://www.roblox.com/asset/?id=162670150"
...
s.SoundId = "rbxassetid://162670113"
s.Parent = o1.PrimaryPart
s.Pitch = 1
s.Volume = 2
s.Looped = true
...
if PlayAudios then
soisoi.SoundId = "rbxassetid://162670113"
soisoi.Volume = 0.3
soisoi.Pitch = 3
soisoi.Looped = true
soisoi:Play()
...
function cMsg(c)
    if SendChatMessages then
        chatremote:FireServer("[Ufo]: " .. c,"All")
    end
end
...
ti(cons, mouse.KeyDown:connect(function(key)
	if key == "b" then
		if fly == false then
			...
			cMsg("Uz ak tan Kontrol! // Remote control!")
		else
			fly = false
			returnhome = false
			cMsg("Bekleme modu. // Standby mode.")
		end
	end
	if key == "w" then
		wd = true
	...
	elseif key == "t" then
		if fly == false and returnhome == false then
			returnhome = true
			cMsg("Host'a Geri Don! // Return The Host!")
	...
	elseif key == "g" then
		if gcd == false and moused == false then
			gcd = true
			if gun == false then
				spawn(function() cMsg("Silah Modu Aktif Ediliyor // Gun Mode Active.") end)
				gun = true
				...
				spawn(function() cMsg("Silah Modu Deaktive Oluyor // Deactivating gun mode.") end)
				gun = false
				for i=1,30 do
					o1.Smg:SetPrimaryPartCFrame(o1.Smg.PrimaryPart.CFrame * CFrame.new(0,-0.01,0))
					wait()
			...
			if math.floor(frms/5) == frms/5 then
				spawn(function()
					...
					if PlayAudios then
    					...
    					s.SoundId = "rbxassetid://387278135"
    					s.Volume = 1
    					s:Play()
    					spawn(function()
    						wait(2)
